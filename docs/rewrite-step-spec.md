...
> 由 [`scripts/score-resume.mjs`](../scripts/score-resume.mjs) 自动校验，超出即 FAIL。

### 硬性上限

- `resumes/working/<name>.md` 与 `resumes/output/<name>.md` **总行数 ≤ 80 行**（含空行）
- 超过即视为不合规，Agent 须**裁剪后重写**，不得绕过 hook

### 禁止「水字数」

简历必须**高信息密度**，禁止以下凑字数行为：

- 通用形容词堆砌（如「热爱技术 / 沟通良好 / 学习能力强 / 抗压能力强」等无证据空话）
- 同义复述：同一战绩或职责在多个章节重复描述
- 罗列工具版本号、依赖列表当 Skills（除非 JD 明确要求该版本）
- Work 段落写成 Project STAR 的复述（Work 须 1–2 行宏观职责）
- 把 `[待补充]` 当占位符堆满段落以撑长度
- 过度使用 markdown 装饰（多余分割线、emoji、引用块）

### 取舍原则

- **删 > 补**：当 Summary 标签 / Skills 维度 / Project 三者无法环环相扣时，**降级或删除**而非硬写
- **精简 > 完整**：JD 无关的旧项目应折叠到一两行 Work 中
- **量化 > 形容**：宁可用 `[待补充: 具体指标]` 也不写「显著提升」「大幅优化」
...
- [ ] **总行数 ≤ 80 行**（`score-resume.mjs` 校验通过）
- [ ] 无通用形容词堆砌、无同义复述、无凑字数装饰
- [ ] 五章节齐全且顺序正确
